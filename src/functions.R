...
remove_monomorphic_markers <- function(geno_df) {
  ...
  } else if ("GID" %in% colnames(geno_df)) {
    # remove genotype column
    geno_df_ <- geno_df[, -match("GID", colnames(geno_df))]
  } else {
    geno_df_ <- geno_df
  }
  ...
  if (length(monomorphic_marker_names) > 0) {
    ...
    if ("Genotype" %in% colnames(geno_df)) {
      ...
      # add genotype column
      geno_df_filtered <- cbind(geno_df$GID, geno_df_filtered)
      colnames(geno_df_filtered)[1] <- "GID"
    }
...
# function which replace patterns with values, note equal length is assumed
replace_pattern <- function(geno_df, vect_pattern_,
                            vect_replace_val_) {
  # verify equality of length
  if (length(vect_pattern_) != length(vect_replace_val_)) {
    stop("vect_pattern_ and vect_replace_val_ must have the same length")
  }
  # replace patterns with values
  for (l in seq_along(vect_pattern_)) {
    pattern_ <- vect_pattern_[l]
    replace_val_ <- vect_replace_val_[l]
    geno_df[geno_df == pattern_] <- replace_val_
  }
  return(geno_df)
}

# function which split a vector of characters and return vector of elements
# before_pattern or after_pattern
get_splitted_elements <- function(vect_char_, pattern_,
                                  which_elem = "after_pattern") {
  if (which_elem == "before_pattern") {
    return(sapply(str_split(vect_char_,
      pattern = pattern_
    ), function(x) x[1]))
  } else {
    return(sapply(str_split(vect_char_,
...
sum_columns_by_pairs <- function(matrix_df) {
  if (ncol(matrix_df) %% 2 != 0) {
    stop("the number of columns has to be even")
  }

  summed_columns <- list()

  for (i in seq(1, ncol(matrix_df), by = 2)) {
    summed_column <- matrix_df[, i] + matrix_df[, i + 1]
    summed_columns[[length(summed_columns) + 1]] <- summed_column
  }

  summed_df <- do.call(cbind, summed_columns)
  colnames(summed_df) <- paste0("sum_", seq(1, ncol(summed_df)))

  return(as.data.frame(summed_df))
}

# function which merge two genotype data frames by geno
merge_by_geno <- function(df1, df2) {
  merge(df1, df2, by = "geno")
}
