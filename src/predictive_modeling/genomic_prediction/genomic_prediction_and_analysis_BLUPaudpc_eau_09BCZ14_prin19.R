# script meant to perform genomic prediction and analyses for apple scab resistance
...
trait_ <- "BLUPaudpc.eau.09BCZ14.prin19"
...
# get phenotype and genotype data
...
# get geno id in order to merge pheno_df and geno_df
id <- get_splitted_elements(
  vect_char_ = geno_df[, 1],
  pattern_ = "/",
  which_elem = "after_pattern"
)
geno_df <- cbind(id, geno_df[, -1])

# merge pheno_df and geno_df for integrity of analyses and slice the merged df
merged_df <- merge(pheno_df, geno_df, by = "id")
pheno_df <- merged_df[, c("id", selected_traits_)]
geno_df <- merged_df[, -match(selected_traits_, colnames(merged_df))]
geno_df <- apply(geno_df[, -1], 2, as.numeric)
...
boxplots_pa_ <- boxplots_pa_ %>%
  layout(
    title = paste0(
      ...
      trait_, ", based on ", ncol(geno_df), " SNPs across ",
...
saveWidget(boxplots_pa_, file = paste0(
  ...
  trait_, "_", ncol(geno_df), "_SNP", ".html"
))
...
saveWidget(barplot_sv_orig, file = paste0(
  output_pred_graphics_path, trait_, "/sv_for_",
  trait_, "_", ncol(geno_df), "_SNP_origin_as_label", ".html"
))
